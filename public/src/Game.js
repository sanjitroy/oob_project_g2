...
BasicGame.Game.prototype = {
	...
	create: function () {
       ...
       this.planet = this.add.sprite(((this.game.world.width/2)),(this.game.world.height/2),'planet');
	...
	updateParticle : function() {
		if(this.nextSparticleAt < this.time.now && this.sparticle.countDead() > 0){
            ...
            if( BasicGame.PLAYER_HEALTH > 0 ){
                spart.reset((this.game.world.width) / 2, (this.game.world.height) / 2);
                //spart.anchor.setTo(0.5,0.5);
                spart.body.velocity.x = this.rnd.integerInRange(-50, 50);
                spart.body.velocity.y = this.rnd.integerInRange(-50, 50);
            }
            //enemy.scale.x = 0.5 ;
            //enemy.scale.y = 0.5 ;
            //bullet.scale.y = 0.3 ;
    ...
    fire : function(x_cor , y_cor){
        ...
        var k = Math.sqrt( (x_cor - this.game.world.width/2)*(x_cor - this.game.world.width/2) + ((y_cor - this.game.world.height/2)*(y_cor - this.game.world.height/2)));
        var new_y =  (y_cor - this.game.world.height/2) / k ;
        var new_x =  (x_cor - this.game.world.width/2) / k ;
        bullet.reset(s_width/2 + new_x, s_height/2 + new_y ) ;
    ...
    spawnEnemey : function(){
        ...
        if(this.nextEnemyAt < this.time.now && this.enemyPool.countDead() > 0){
            ...
            var chr = this.rnd.integerInRange( 1 , 4 ) ;
            if(chr == 1 && BasicGame.PLAYER_HEALTH>0){
                enemy.reset(this.rnd.integerInRange( 10, s_width - 10 ) , 10 , BasicGame.RED_ENEMY_HEALTH) ;
            }
            else
            if(chr == 2 && BasicGame.PLAYER_HEALTH>0){
                enemy.reset( 10 , this.rnd.integerInRange( 10 , s_height - 10 ) , BasicGame.RED_ENEMY_HEALTH) ;
            }
            else
            if(chr == 3 && BasicGame.PLAYER_HEALTH>0){
                enemy.reset( s_width - 10 , this.rnd.integerInRange( 10 , s_height - 10 ) , BasicGame.RED_ENEMY_HEALTH) ;
            }
            else
            if(BasicGame.PLAYER_HEALTH>0){
                enemy.reset( this.rnd.integerInRange( 10 , s_width - 10 ) , s_height - 10 , BasicGame.RED_ENEMY_HEALTH );
            }
        ...
        if(this.nextblue_EnemyAt < this.time.now && this.blue_enemyPool.countDead() > 0){
            ...
            var chb = this.rnd.integerInRange( 1 , 4 );
            if(chb == 1 && BasicGame.PLAYER_HEALTH>0){
                blue_enemy.reset(this.rnd.integerInRange( 10, s_width - 10 ) , 10 , BasicGame.RED_ENEMY_HEALTH) ;
            }
            else
            if(chb == 2 && BasicGame.PLAYER_HEALTH>0){
                blue_enemy.reset( 10 , this.rnd.integerInRange( 10 , s_height - 10 ) , BasicGame.RED_ENEMY_HEALTH) ;
            }
            else
            if(chb == 3 && BasicGame.PLAYER_HEALTH>0){
                blue_enemy.reset( s_width - 10  , this.rnd.integerInRange( 10 , s_height - 10 ) , BasicGame.RED_ENEMY_HEALTH) ;
            }
            else
            if(BasicGame.PLAYER_HEALTH>0){
                blue_enemy.reset( this.rnd.integerInRange( 10 , s_width - 10 ) , s_height - 10 , BasicGame.RED_ENEMY_HEALTH );
            }
        ...
        if(this.nextgreen_EnemyAt < this.time.now && this.green_enemyPool.countDead() > 0){
            ...
            var chg = this.rnd.integerInRange( 1 , 4 );
            if(chg == 1 && BasicGame.PLAYER_HEALTH>0){
                green_enemy.reset(this.rnd.integerInRange( 10, s_width - 10 ) , 10 , BasicGame.RED_ENEMY_HEALTH) ;
            }
            else
            if(chg == 2 && BasicGame.PLAYER_HEALTH>0){
                green_enemy.reset( 10 , this.rnd.integerInRange( 10 , s_height - 10 ) , BasicGame.RED_ENEMY_HEALTH) ;
            }
            else
            if(chg == 3 && BasicGame.PLAYER_HEALTH>0){
                green_enemy.reset( s_width - 10  , this.rnd.integerInRange( 10 , s_height ) , BasicGame.RED_ENEMY_HEALTH) ;
            }
            else
            if(BasicGame.PLAYER_HEALTH>0){
                green_enemy.reset( this.rnd.integerInRange( 10 , s_width - 10 ) , s_height - 10 , BasicGame.RED_ENEMY_HEALTH );
            }
    ...
    gravity_check : function(){
        ...
        **/
        ...
        this.laserPool.forEachAlive(function(bullet){
            var k = Math.sqrt( (bullet.x - s_width/2)*(bullet.x - s_width/2) + ((bullet.y - s_height/2)*(bullet.y - s_height/2)));
        bullet.body.velocity.y = 50 * (bullet.y - s_height/2) / k ;
        bullet.body.velocity.x = 50 * (bullet.x - s_width/2) / k ;
    ...
    enemy_velocity_check: function(enemy){
        var k = Math.sqrt( (enemy.x - (s_width/2) )*(enemy.x - (s_width/2) ) + (enemy.y - (s_height/2))*(enemy.y - (s_height/2)) ) ;
        if( k < 200 && BasicGame.PLAYER_HEALTH>0 ){
                enemy.body.velocity.x = -(50 * (enemy.x - (s_width/2))) / k ;
                enemy.body.velocity.y = -(50 * (enemy.y - (s_height)/2)) / k ;
                //enemy.body.velocity.y += 10 ;
                /*var k=(s_height-enemy.y)/(enemy.x-(s_width/2));
                if(enemy.x>s_width/2 && BasicGame.PLAYER_HEALTH>0){
                    enemy.body.velocity.x = -1000/Math.sqrt(1+k*k);
